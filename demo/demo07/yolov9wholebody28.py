...
def postprocess_subclass(
    ...
    if len(boxes) > 0:
        ...
        if len(boxes_keep) > 0:
            ...
            # Object filter (Wholebody28: 0,5,6,7,16,17,18,19,20,23,24,25,27)
            result_boxes = [
                box for box in result_boxes \
                    if (box.classid in [0,5,6,7,16,17,18,19,20,23,24,25,27] and box.score >= obj_class_score_th) or box.classid not in [0,5,6,7,16,17,18,19,20,23,24,25,27]
            ]
            # Attribute filter
            result_boxes = [
                box for box in result_boxes \
            ...
            # Keypoint filter (Wholebody28: 21,22,26)
            result_boxes = [
                box for box in result_boxes \
                    if (box.classid in [21,22,26] and box.score >= obj_class_score_th) or box.classid not in [21,22,26]
            ]
            ...
            if not disable_left_and_right_hand_identification_mode:
                ...
                left_right_hand_boxes = [
                    box for box in result_boxes if box.classid in [24, 25]
                ]
                find_most_relevant_obj(base_objs=hand_boxes,
                                       target_objs=left_right_hand_boxes)
            result_boxes = [
                box for box in result_boxes if box.classid not in [24, 25]
            ]
    return result_boxes
...
def draw_debug(
    ...
    for box in boxes:
        ...
        if (classid == 0 and not disable_gender_identification_mode) \
            ...
            # Keypoints: Elbow, Wrist, Foot - draw as red dots (fixed 16px diameter)
            center_x = (box.x1 + box.x2) // 2
            center_y = (box.y1 + box.y2) // 2
            radius = 8  # 16px diameter
            cv2.circle(debug_image, (center_x, center_y), radius, (255, 255, 255), -1)
            cv2.circle(debug_image, (center_x, center_y), radius - 1, (0, 0, 255), -1)
