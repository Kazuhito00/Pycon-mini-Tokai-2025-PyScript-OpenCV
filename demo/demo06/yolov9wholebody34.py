...
def postprocess_nms(
    ...
    # 各クラスごとのスコアとクラスIDを取得
    class_ids: np.ndarray = np.argmax(scores, axis=1)
    confidences: np.ndarray = np.max(scores, axis=1)
...
def postprocess_subclass(
    image: np.ndarray,
    obj_class_score_th: float,
    attr_class_score_th: float,
    keypoint_th: float,
    boxes: np.ndarray,
    ...
    box_score_threshold: float = min([obj_class_score_th, attr_class_score_th, keypoint_th])
        ...
        if len(boxes_keep) > 0:
            ...
            result_boxes = [
                box for box in result_boxes \
                    if (box.classid in [0,5,6,7,16,17,18,19,20,26,27,28,33] and box.score >= obj_class_score_th) or box.classid not in [0,5,6,7,16,17,18,19,20,26,27,28,33]
            ]
            # Attribute filter
            result_boxes = [
                box for box in result_boxes \
            ...
            result_boxes = [
                box for box in result_boxes \
                    if (box.classid in [21,22,23,24,25,29,30,31,32] and box.score >= keypoint_th) or box.classid not in [21,22,23,24,25,29,30,31,32]
            ]
            ...
            # classid: 26 -> Hand (changed from 21 in wholebody25)
            #   classid: 27 -> Left-Hand (changed from 22 in wholebody25)
            #   classid: 28 -> Right-Hand (changed from 23 in wholebody25)
            ...
            for target_classid in [21,22,23,24,25,29,30,31,32]:
                ...
                filtered_keypoints_boxes = nms(target_objs=keypoints_boxes, iou_threshold=0.20)
...
def nms(
    *,
    ...
    target_objs = sorted(target_objs, key=lambda x: x.score, reverse=True)

    keep_boxes = []
    while len(target_objs) > 0:
        # Keep the box with highest score
        current_box = target_objs[0]
        keep_boxes.append(current_box)
        target_objs = target_objs[1:]

        # Remove boxes with high IoU
        filtered_boxes = []
        for box in target_objs:
            iou = calculate_iou(base_obj=current_box, target_obj=box)
            if iou < iou_threshold:
                filtered_boxes.append(box)
        target_objs = filtered_boxes
...
def draw_debug(
    ...
    for box in boxes:
        ...
        if classid == 0:
            ...
            # Hand (changed from 21 in wholebody25)
            if not disable_left_and_right_hand_identification_mode:
                # Hands
                if box.handedness == 0:
                    # Left-Hand
        ...
        # Draw keypoints differently based on mode
        if classid in [21,22,23,24,25,29,30,31,32]:
            # Keypoints
            if keypoint_drawing_mode == 'box':
            ...
            elif keypoint_drawing_mode == 'dot':
                ...
                cv2.rectangle(debug_image, (box.x1, box.y1), (box.x2, box.y2), (255,255,255), 2)
                cv2.rectangle(debug_image, (box.x1, box.y1), (box.x2, box.y2), color, 1)
        elif (classid == 0 and not disable_gender_identification_mode) \
        ...
        # Attributes text (only for Body and Head)
        if classid in [0, 7, 26]:
            generation_txt = ''
            if box.generation == -1:
                generation_txt = ''
            ...
            # Combine all text
            if attr_txt and handedness_txt:
                final_txt = f'{attr_txt} {handedness_txt}'
            elif attr_txt:
                final_txt = attr_txt
            elif handedness_txt:
                final_txt = handedness_txt
            else:
                final_txt = ''
